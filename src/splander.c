...
static void UpdateServer(GameState *gs)
{
	...
	sprintf(sent_time_str, "%d:%d:%d:%f", hour, minute, second, millisec);
	...
	char x[5];
	char y[5];
	sprintf(x, "%.2f", gs->player.pos.x);
	sprintf(y, "%.2f", gs->player.pos.y); // TODO not work
	// strcat(send_data, x);
	// strcat(send_data, ":");
	// strcat(send_data, y);

	// put all syncy data here  V
	if (sendto(client_socket, send_data, send_len, 0, (struct sockaddr *)&server, sizeof(server)) == SOCKET_ERROR)
	{
		printf("sendto() failed. Error: %d\n", WSAGetLastError());
		closesocket(client_socket);
		WSACleanup();
